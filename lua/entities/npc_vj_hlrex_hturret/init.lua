...
-----------------------------------------------*/
ENT.Model = {"models/vj_hlr/hl_ex/hturret.mdl"} -- The game will pick a random model from the table when the SNPC is spawned | Add as many as you want
ENT.StartHealth = 200
...
function ENT:CustomOnThink()
	...
	if parameter != self.Turret_CurrentParameter then
		self.turret_turningsd = CreateSound(self, "vj_hlr/hl1_npc/turret/motor_loop.wav") 
...
function ENT:CustomOnThink_AIEnabled()
	if IsValid(self:GetEnemy()) or self.Alerted == true then
		...
		if CurTime() > self.NextResetEnemyT && self.Alerted == false && self.Turret_StandDown == false then
			...
			VJ_EmitSound(self,{"vj_hlr/hl1_npc/turret/tu_retract.wav"}, 70, 100)
		end
...
function ENT:CustomOn_PoseParameterLookingCode(pitch, yaw, roll)
	...
	if (math.abs(math.AngleDifference(self:GetPoseParameter("aim_yaw"), math.ApproachAngle(self:GetPoseParameter("aim_yaw"), yaw, self.PoseParameterLooking_TurningSpeed))) >= 10) or (math.abs(math.AngleDifference(self:GetPoseParameter("aim_pitch"), math.ApproachAngle(self:GetPoseParameter("aim_pitch"), pitch, self.PoseParameterLooking_TurningSpeed))) >= 10) then
		...
		if self.Turret_HasLOS == false && IsValid(self:GetEnemy()) then -- If it just got LOS, then play the gun "activate" sound
			VJ_EmitSound(self,{"buttons/blip1.wav"}, 85, 100)
		end
...
function ENT:CustomOnAlert(ent)
	...
	VJ_EmitSound(self,{"vj_hlr/hl1_npc/turret/tu_deploy.wav"}, 70, 100)
	self.turret_alertsd = VJ_CreateSound(self, "vj_hlr/hl1_npc/turret/tu_alert.wav", 75, 100)
...
function ENT:CustomRangeAttackCode()
	...
	VJ_EmitSound(self, {"vj_hlr/hl2ex_weapon/mp5_fire.mp3"}, 90, self:VJ_DecideSoundPitch(70,75))
	VJ_EmitSound(self, {"vj_hlr/hl2ex_weapon/mp5_fire_dist.mp3"}, 150, self:VJ_DecideSoundPitch(75,85))
	
	-- Effects & Light
	ParticleEffect("vj_rifle_full", startpos, self:GetAngles(), self)
